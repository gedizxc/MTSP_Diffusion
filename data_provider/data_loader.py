...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path),parse_dates=['date'])
        ...
        df_raw[cols] = data
        feature = pd.DataFrame(df_raw[cols][border1:border2]) #不含时间戳的信息
        #STL


        STL_Date_index = df_raw[['date']][border1:border2]    #时间戳信息
        STL_Data = pd.DataFrame.join(STL_Date_index,feature)
    ...
    def stl(self,STL_Data):
        ...
        for i ,col in enumerate(cols):
            df = STL_Data[col]
            res = STL(df).fit()
            #res = STL(df, period=6 * 24).fit()  # weather：6*24，ettm1:4x24
    ...
    def __getitem__(self, index):
        ...
        return seq_x, seq_y, seq_x_mark, seq_y_mark,stl_data
